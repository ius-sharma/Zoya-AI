...
export const ParticleOrb: React.FC<ParticleOrbProps> = ({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // --- 1. CLIP TO PERFECT CRISP CIRCLE (Clean Edge, Zero Outer Glow) ---
      ctx.save();
      ctx.beginPath();
      ctx.arc(centerX, centerY, circleRadius, 0, Math.PI * 2);
      ctx.clip();

      // BASE: Pure Satin Cream Silk Foundation
      ctx.fillStyle = '#FDFBF7';
      ctx.fillRect(
        centerX - circleRadius - 5,
        centerY - circleRadius - 5,
        (circleRadius + 5) * 2,
        (circleRadius + 5) * 2
      );

      // --- 2. INTERNAL LIQUID GRADIENT SWIRL (Cream + Rust Brown Palette) ---
      const t = time * swirlSpeed;

      // Fluid Node 1: Primary Rich Rust Brown Cloud (#9C4A1A / #8B3A0F)
      const n1Angle = t * 0.9;
      ...
      grad1.addColorStop(0, 'rgba(156, 74, 26, 0.96)'); // Rich Rust Brown
      grad1.addColorStop(0.38, 'rgba(184, 93, 25, 0.85)'); // Terracotta Rust
      grad1.addColorStop(0.72, 'rgba(245, 235, 224, 0.40)'); // Warm Silk Cream
      grad1.addColorStop(1, 'rgba(253, 251, 247, 0)');
      ...
      // Fluid Node 2: Warm Terracotta Caramel Swirl (#C25E1A / #D97706)
      ...
      grad2.addColorStop(0, 'rgba(194, 94, 26, 0.90)'); // Warm Terracotta
      grad2.addColorStop(0.40, 'rgba(217, 119, 6, 0.75)'); // Amber Gold
      grad2.addColorStop(0.75, 'rgba(245, 235, 224, 0.30)');
      grad2.addColorStop(1, 'rgba(253, 251, 247, 0)');
      ...
      // Fluid Node 3: Deep Espresso Rust Accent (#7C3512)
      ...
      grad3.addColorStop(0, 'rgba(124, 53, 18, 0.85)'); // Espresso Rust
      grad3.addColorStop(0.50, 'rgba(156, 74, 26, 0.40)');
      grad3.addColorStop(1, 'rgba(253, 251, 247, 0)');
      ...
      // Fluid Node 4: Soft Warm Ivory Core (#F5EBE0)
      ...
      coreGrad.addColorStop(0, 'rgba(255, 255, 255, 0.98)'); // Pearl White
      coreGrad.addColorStop(0.45, 'rgba(245, 235, 224, 0.75)'); // Satin Cream
      coreGrad.addColorStop(0.80, 'rgba(232, 216, 200, 0.25)');
      coreGrad.addColorStop(1, 'rgba(253, 251, 247, 0)');
      ...
      // Fluid Node 5: Satin Silk Luminous Highlight Edge
      ...
      hlGrad.addColorStop(0.55, 'rgba(245, 235, 224, 0.45)');
      hlGrad.addColorStop(1, 'rgba(253, 251, 247, 0)');
      ...
      // --- 3. CRISP CLEAN CIRCULAR RIM (No Shadow / No Outer Glow) ---
      ctx.beginPath();
      ctx.arc(centerX, centerY, circleRadius, 0, Math.PI * 2);
      ctx.lineWidth = 1.2;
      ctx.strokeStyle = 'rgba(156, 74, 26, 0.35)';
      ctx.stroke();
